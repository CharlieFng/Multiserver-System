...
public class Client {
	...
	public static void main(String[] args) {
		...
		options.addOption("srp",true,"secure remote port number");
		options.addOption("rh",true,"remote hostname");
		options.addOption("s",true,"secret for username");
		options.addOption("ss",true,"self-defined secret");
		options.addOption("a",true,"register or login");
		...
		if(cmd.hasOption("srp")){
			try{
				int port = Integer.parseInt(cmd.getOptionValue("srp"));
				Settings.setSecureRemotePort(port);
			} catch (NumberFormatException e){
				log.error("-srp requires a port number, parsed: "+cmd.getOptionValue("srp"));
				help(options);
			}
		}
		...
		if(cmd.hasOption("ss")){
			Settings.setSelfSecret(cmd.getOptionValue("ss"));
		}
		if(cmd.hasOption("a")){
			Settings.setAction(cmd.getOptionValue("a"));
		}
